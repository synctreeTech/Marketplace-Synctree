...
Set admin database password within '' as in Example.
```
export ADMIN_PASSWORD='YourPassword'
sed -i '/^export ADMIN_PASSWORD=/d' ~/env.sh
echo "export ADMIN_PASSWORD=${ADMIN_PASSWORD}" >> ~/env.sh
sed -i '/^admin_password:/d' ~/synctree.yaml
echo "admin_password: ${ADMIN_PASSWORD}" >> /home/ec2-user/synctree.yaml

PW_ESCAPED=$(printf '%s' "$ADMIN_PASSWORD" | sed 's/[&\\|]/\\&/g')
SED_EXPR=$(printf 's|%s|%s|g' 'Dpsxjvmf12!' "$PW_ESCAPED")
sed -i "$SED_EXPR" /home/ec2-user/install-synctree/sql/*.sql
```

Example:

```
export ADMIN_PASSWORD='Xptmxm12!@'
sed -i '/^export ADMIN_PASSWORD=/d' ~/env.sh
echo "export ADMIN_PASSWORD=${ADMIN_PASSWORD}" >> ~/env.sh
sed -i '/^admin_password:/d' ~/synctree.yaml
echo "admin_password: ${ADMIN_PASSWORD}" >> /home/ec2-user/synctree.yaml
